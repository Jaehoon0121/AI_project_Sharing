...
def attitudeEuler_6DOF(eta,nu,sampleTime):
    """
    eta = attitudeEuler_6DOF(eta,nu,sampleTime) computes the generalized 
    ...
    p_dot   = np.matmul( Rzyx(l[3], l[4], l[5]), nu[0:3] )
    v_dot   = np.matmul( Tzyx(l[3], l[4]), nu[3:6] )

    # Forward Euler integration
    l[0:3] = l[0:3] + sampleTime * p_dot
    l[3:6] = l[3:6] + sampleTime * v_dot

    return l

#------------------------------------------------------------------------------

def attitudeEuler_3DOF(eta,nu,sampleTime):
    """
    eta = attitudeEuler_3DOF(eta,nu,sampleTime) computes the generalized 
    position/Euler angles eta[k+1]
    """
    
    p_dot   = np.array([nu[0] * np.cos(eta[2]) - nu[1] * np.sin(eta[2]), nu[0] * np.sin(eta[2]) + nu[1] * np.cos(eta[2])])
