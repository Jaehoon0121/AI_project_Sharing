...
import numpy as np
from .gnc import attitudeEuler_3DOF, attitudeEuler_6DOF
...
def simulate(N, sampleTime, vehicle):
    ...
    for i in range(0,N+1):
        ...
        eta = attitudeEuler_6DOF(eta,nu,sampleTime)
...
###############################################################################
# Function calculate_6DOF(sampleTime, vehicle, u_control, eta)
###############################################################################
def calculate_6DOF(sampleTime, vehicle, u_control, eta = np.array([0, 0, 0, 0, 0, 0], float), nu = np.array([0, 0, 0, 0, 0, 0], float)):

    # Initial state vectors
    location = eta.copy()    # position/attitude, user editable
    velocity = nu.copy()      

    u_control = u_control

    v_prime = vehicle.dynamics(location,velocity,u_control,sampleTime)
    l_prime = attitudeEuler_6DOF(location, v_prime, sampleTime)

    return v_prime, l_prime


###############################################################################
# Function calculate_3DOF(sampleTime, vehicle, u_control, eta)
###############################################################################
def calculate_3DOF(sampleTime, vehicle, u_control, eta = np.array([0, 0, 0], float), nu = np.array([0, 0, 0], float)):
    ...
    v_prime = vehicle.dynamics(location,velocity,u_control,sampleTime)
    l_prime = attitudeEuler_3DOF(location, v_prime, sampleTime)
